...
#define NET_MAX_PACKET_PAYLOAD_SIZE (NET_MAX_PACKET_SIZE - sizeof(Net_PacketHeader))
#define NET_MAX_PACKET_CHAIN_LENGTH 16
...
{
    Uint64 tick_id;
    Uint8 packet_buf[NET_MAX_PACKET_CHAIN_LENGTH * NET_MAX_PACKET_PAYLOAD_SIZE];
...
static S8 Net_GetChainBuffer(Net_PacketChain *chain, Uint32 packet_index)
{
    ...
    {
        res = S8_Make(chain->packet_buf + packet_index*NET_MAX_PACKET_PAYLOAD_SIZE,
                      NET_MAX_PACKET_PAYLOAD_SIZE);
    }
    return res;
}
